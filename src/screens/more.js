...
export default class SignUpScreen extends React.Component {
  ...
  componentDidMount() {
    console.log(
      'The params got' +
        this.props.navigation.getParam('name', 'were not recvd'),
    );
  }
  getLocation = () => {
  ...
  uploadImage = (uri, mime = 'application/octet-stream') => {
    ...
    const imageRef = firebase
      ...
      .ref('/images/')
      .child(
        this.props.navigation.getParam('name', 'Unnamed') + '_' + sessionId,
      );
  ...
  upDate = () => {
    if (this.state.imageCount == 0) {
      ...
      this.state.db
        .collection('Users')
        .doc(this.props.navigation.getParam('name', 'Unnamed'))
        .set({
  ...
  check = () => {
    ...
    if (this.state.consition != '') this.setState({check: true});
  ...
  render() {
    return (
      <ImageBackground
        source={require('/Users/apple/trana2/src/assets/backgroundimage.jpg')}
